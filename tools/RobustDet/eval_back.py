...
def do_python_eval(output_dir='output', use_07=True):
    ...
    return np.round(np.mean(aps), decimals=3)  # by lyz
...
def test_net_attack(net, dataset, corruption=None, corruption_severity=None):
    ...
    if args.dataset=='VOC':
        return evaluate_detections_voc(all_boxes, output_dir, dataset)
    else:
        return evaluate_detections_coco(all_boxes, output_dir, dataset)
...
def evaluate_detections_coco(box_list, output_dir, dataset):
    ...
    # by lyz COCOEvaluator().evaluate() write to results.json and read from it again, multi-gpu conflict
    mean_ap, detail = evaluate_coco(image_ids, bboxes, classes, probs)

    logger.info(f'mean AP: {mean_ap}')
    logger.info(detail)
    return np.round(mean_ap, decimals=3)
