...
VOC_ROOT = '/mnt/data/causal_rodc/data/'
COCO_ROOT = '/mnt/data/causal_rodc/data/coco2017/'
...
parser.add_argument('--augmix', default=False, type=str2bool,
                    help='Use AugMix')
...
logger = get_logger(os.path.join(args.log_folder, f'train_{task_name}_{args.optimizer}.log'))
...
def train():
    if args.dataset == 'COCO':
        ...
        dataset = VOCDetection(root=args.dataset_root, image_sets=image_sets,
                               transform=SSDAugmentation(cfg['min_dim'], augmix=args.augmix))# by lyz, load_sizes=True)
    ...
    if not args.resume and args.basenet=='None':  # by lyz
    ...
    if args.optimizer == 'SGD':  # by lyz
        optimizer = optim.SGD(net.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    elif args.optimizer == 'SAM':
        optimizer = SAM(net.parameters(), optim.SGD, rho=args.rho, adaptive=False, lr=args.lr, momentum=args.momentum,
                        weight_decay=args.weight_decay)
    elif args.optimizer == 'ASAM':
        optimizer = SAM(net.parameters(), optim.SGD, rho=args.rho, adaptive=True, lr=args.lr, momentum=args.momentum,
                        weight_decay=args.weight_decay)
    ...
    data_loader = data.DataLoader(dataset, args.batch_size,
                                  ...
                                  pin_memory=True, generator=torch.Generator(device='cuda'))  # by lyz pin_memory=True))

    # create batch iterator
    t0 = time.time()
    loss_sum=0
    batch_iterator = BatchIter(data_loader, args.cuda)
    for iteration in range(args.start_iter, args.max_iter):  # by lyz cfg['max_iter']):
        # if args.visdom and iteration != 0 and (iteration % epoch_size == 0):  # by lyz
