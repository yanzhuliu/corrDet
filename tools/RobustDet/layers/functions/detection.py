...
class Detect(Function):
    ...
    """
    '''def __init__(Detect, num_classes, bkg_label, top_k, conf_thresh, nms_thresh):
    ...
    @staticmethod
    def set(num_classes, bkg_label, top_k, conf_thresh, nms_thresh):
        Detect.num_classes = num_classes
        Detect.background_label = bkg_label
        Detect.top_k = top_k
        # Parameters used in nms.
    ...
    @staticmethod
    def forward(ctx, loc_data, conf_data, prior_data, **kwargs: None):
        """
        Args:
        ...
        for i in range(num):
            ...
            for cl in range(1, Detect.num_classes):
                ...
                ids_list[cl] = ids
                count_list[cl] = count
                cmask_list[cl] = c_mask
                score_list[cl] = scores
                output[i, cl, :count] = torch.cat((scores[ids[:count]].unsqueeze(1), boxes[ids[:count]]), 1)
            #torch.save({'ids': ids_list, 'count': count_list, 'c_mask': cmask_list, 'scores': score_list}, './id_data_clean.pth')
            Detect.obj_dict={'ids': ids_list, 'count': count_list, 'c_mask': cmask_list, 'scores': score_list}
